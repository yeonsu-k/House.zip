<template>
  <div class="container">
    ...
    <div v-else>
      <b-button style="width: 99%" variant="outline-success" class="btn" :to="{ name: 'QnaModify', params: { no: qna.no } }">질문수정</b-button>
      <hr />
      <h6 align="center">아직 답변이 달리지 않았습니다.</h6>
      <b-button variant="outline-warning  " style="width: 99%" class="btn" :to="{ name: 'QnaRegistAns', params: { no: qna.no } }">답변작성</b-button>
    </div>
    <b-button variant="outline-success " style="width: 99%" class="btn" @click="deleteQna">질문삭제</b-button>
  </div>
</template>
<script>
